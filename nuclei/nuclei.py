...
NUCLEI_TEMPLATES_PATH = pathlib.Path("./templates/").absolute()
NUCLEI_TEMPLATES_MOUNTPOINT = pathlib.Path("/root/nuclei-templates")  # Cannot change
STORAGE_MOUNTPOINT = "/root/output"
...
class Nuclei(ScanModule):
    ...
    def enqueue(self, taskcfg: NucleiTask) -> None:
        ...
        assert self.running
        outfp = self.config.storage_path / taskcfg.label
        ...
        try:
            command: list[str] = [
                ...
                "-no-color",
                "-json-export",
                f"{STORAGE_MOUNTPOINT}/output.json",
                "-disable-update-check",
            ]
            if taskcfg.template is not None:
                command.extend(["-t", taskcfg.template])
            ctx = self.docker.containers.run(
                ...
                stdout=True,
                stderr=True,
                labels={NUCLEI_TASK_LABEL_KEY: taskcfg.label},
                volumes={
                    ...
                    str(outfp): {
                        "bind": str(STORAGE_MOUNTPOINT),
                        "mode": "rw",
                    },
                },
            )
        except docker.errors.APIError as e:
            logging.error("Nuclei execution failed: %s", str(e))
            self.config.callback(taskcfg.label, False)
            return
        with self.lock:
            self.containers.add((ctx, taskcfg))

    def shutdown(self, wait: bool = True) -> None:
        logging.info("Shutting down (wait=%s)", wait)
        self.running = False
    ...
    def update_templates(self) -> None:
        ...
        try:
            ...
            logging.error("Failed to update templates: %s", str(e))
            raise
        runtime = time.time() - start
        self.logger.info("Updated templates in %.03fs", runtime)
    ...
    def handle_finished_containers(self) -> None:
        ...
        with self.lock:
            for ctx, cfg in self.containers:
                ...
                outfp = self.config.storage_path / cfg.label
                ...
                with gzip.open(outfp / "stderr.txt.gz", "wt", encoding="utf8") as fd:
                    fd.write(stderr)
                self.config.callback(cfg, True)
                logging.info(
                    "Nuclei task completed, id %s status %s",
                    cfg.label,
                    DockerContainer.get_status_code(r),
                )
