...
IpTimeSeries = list[tuple[dt.datetime, str, int]]
"""
List of time series entries, each as a tuple (timestamp, fingerprint, port)
"""


# Path used in fingerprint field
FieldPath = list[str]
"""
Path to a fingerprint field after splitting the original path by dots
"""
...
class ModuleData:
    def __init__(
        ...
        self.fingerprintField: FieldPath = fieldPath.split(sep=".")
...
def initSupportedModules() -> dict[str, ModuleData]:
    ...
    modData: ModuleData = ModuleData(
        ...
        fieldPath="ssl.cert.fingerprint.sha256"
    )
...
def getNestedFieldData(scan: dict[str, Any], modData: ModuleData) -> str | None:
    """
    ...
    Returns
    -------
    ``data``: fingerprint data after it has been processed by fingerprintExtractor (if available)
    """
...
def initParser(supportedModules: dict[str, ModuleData]) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="saves dynamic/static IP addresses found as a .pickle file (default: %(default)s)",
    )
...
def getFingerprintsAndIps(
    ...
    for file in os.scandir(args.shodanDir):
        ...
        try:
            ...
            logging.error(
                f"Data from file {file.name} doesn't match the expected structure: list[dict[str, Any]]. Aborted"
            )
            exit(2)
        ...
        for scan in data:
            try:
                ...
                logging.warning(
                    f"Required field '{ke.args[0]}' not found in Shodan scan. Skipping"
                )
                continue
    ...
    if len(fingerprintsPerIp) == 0 and len(ipsPerFingerprint) == 0:
        logging.error(
            f"No compatible files found in directory '{args.shodanDir}'. Please ensure that scans are .json files and are formatted correctly"
        )
        exit(3)
...
def findStaticAndDynamicIps(
    ...
    for ip in possibleDynamicIps:
        timeSeries: list[tuple[dt.datetime, str, int]] = sorted(
        ...
        for timestamp, fingerprint, port in timeSeries:
            ...
            if fingerprint != currentFingerprintSeenByPort[port]:
                if fingerprint in seenFingerprintsPerPort[port]:
                    # Fingerprint is not new and has been seen before, so at least one fingerprint for this port has the pattern bbbaaabbb
                    ...
                    seenFingerprintsPerPort[port].add(
                        currentFingerprintSeenByPort[port]
                    )
...
if __name__ == "__main__":
    ...
    staticIps, dynamicIps = findStaticAndDynamicIps(
        fingerprintsPerIp, ipsPerFingerprint, ipFingerprintsOverTime
    )
    ...
    logging.info(
        f"Static IP addresses ratio {100 * len(staticIps) / (len(dynamicIps) + len(staticIps)):.6f}%"
    )
    ...
    if args.shouldSaveIps:
        ...
        logging.info(
            "Static and Dynamic IP addresses have been written to static_ips.pickle and dynamic_ips.pickle respectively"
        )
